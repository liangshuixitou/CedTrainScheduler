...
class FCFSDataScheduler(SchedulerBase):
    def __init__(self, cluster_manager: ClusterManager, task_record: Record, file_system: FileSystem):
        super().__init__(SchedulerType.FCFS_DATA, cluster_manager, task_record, file_system)
        self.queue_policy = FCFSQueuePolicy(cluster_manager, task_record, file_system, self.task_queue)
        self.central_policy = DataAffinityPolicy(cluster_manager, task_record, file_system)
